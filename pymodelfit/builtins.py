...
class GaussianModel(FunctionModel1DAuto):
    ...
    _FWHMFACTOR = 2*(2*np.log(2))**0.5  # a constant
    def _getFWHM(self):
        return self.sig*GaussianModel._FWHMFACTOR
    def _setFWHM(self,val):
        self.sig = val/GaussianModel._FWHMFACTOR
...
class VoigtModel(GaussianModel,LorentzianModel):
    ...
    def _getFWHM(self):
        fwhm_g = self.sig * GaussianModel._FWHMFACTOR  # gaussian FWHM
        fwhm_l = 2 * self.gamma
        return 0.5346 * fwhm_l + (0.2166 * fwhm_l**2 + fwhm_g**2)**0.5
    def _setFWHM(self, val):
        raise NotImplementedError('setting FWHM for a voigt profile is ambiguous')
    FWHM = property(_getFWHM,_setFWHM,doc='Full Width at Half Maximum (Voigt approximation from Olivero&Longbothum 77)')
